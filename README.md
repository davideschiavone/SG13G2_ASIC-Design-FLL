...
> Computing, Johannes Kepler University Linz), used here as the **development scaffold**
> (LibreLane macro hardening, Xschem analog entry, per-macro DRC/LVS/PEX). It previously
> used JKU's ORFS-based *SG13G2_ASIC-Design-Template* (also by Simon Dorrer); that
> `.xspice` co-sim reference is preserved under [`reference/orfs-xspice/`](reference/orfs-xspice/).
> The original template's documentation, credits, and citation are retained in full below.
>
> **Note on the top-level vehicle:** the template's flow produces its own QFN-32
> 1.6×1.6 mm padframe chip. That padframe is **not** the FLL's final vehicle — the FLL
> targets a **TinyTapeout analog tile**, into which the hardened FLL macros are adapted
> at top level. See [`CLAUDE.md`](CLAUDE.md) for the project plan and constraints.
